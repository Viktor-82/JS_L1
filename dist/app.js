class Board {
     ...
     */
    renderBoard() {
        this.boardEl.innerHTML = '';// очищаем игровое поле
        for (let row = 0; row < this.settings.rowsCount; row++) {
            let tr = document.createElement('tr');// создаем строку
            this.boardEl.appendChild(tr);// добавляем строку к игровому полю

            for (let col = 0; col < this.settings.colsCount; col++) {
                let td = document.createElement('td');// создаем колонку
                tr.appendChild(td);// добавляем колонку в строку
            }
        }
    }
...
class Game {
     ...
     */
    run() {
        /* Метод .bind позволяет менять this. Прописывать чему он будет равен. 
        Создает новую функцию точно такую же как this.start но this прописывается другой.
        В данном случае this изначально указывал на button, но переписываем его на объект 
        в котором он находится (Game) */
...
/* Метод .assign() объекта берет передаваемые параметры params и 
заменяет их в объекте defaultParams, другие параметры оставляет как есть */
